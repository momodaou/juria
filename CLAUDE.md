...
- Modules développés (suite) : Cabinet/RH (équipe et charge de travail, congés avec validation, pointage/compteur d'heures mensuel, échéances RH — fin d'essai/contrat/visite médicale via la vue `v_echeances_rh` déjà prête, bulletins de paie en archivage indicatif). Nouvelle table `conges` (le schéma avait `presences` et `bulletins_paie` mais pas de table congés).
- Modules restant à développer (backend + écrans) : Assistant IA (écran dédié), Portail client. Voir `APP/README.md` § 5-6.
