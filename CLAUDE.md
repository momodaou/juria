...
- Modules développés (suite) : Plan d'action (kanban à faire/en cours/à valider/terminé, sur la table `taches` déjà prévue par le kit).
- Modules restant à développer (backend + écrans) : Dépenses & caisse, Rétrocessions, Accès & permissions, Cabinet (RH), Assistant IA (écran dédié), Portail client. Voir `APP/README.md` § 5-6.
