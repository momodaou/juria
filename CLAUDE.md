...
- **Changement de mot de passe en libre-service** (17/08/2026) : nouvelle route `GET/PUT /api/profil` (`backend/src/routes/profil.js`, montée sans restriction de rôle — accessible à tout utilisateur authentifié, contrairement à `/api/acces`). `PUT /api/profil/mot-de-passe` exige l'ancien mot de passe, ≥8 caractères, différent de l'ancien. Écran `Mon compte` (`pages/mon-compte/`) accessible depuis le menu par tous.
  - ⚠️ **Gap réel non comblé** : aucun flux « mot de passe oublié ». Ce changement de mot de passe exige de connaître l'ancien — si un utilisateur le perd avant toute connexion (ou l'oublie plus tard), **il n'existe aujourd'hui aucun moyen de le récupérer ni de le réinitialiser**, pas même pour un associé/admin (`/api/acces` n'a pas de route « réinitialiser le mot de passe d'un tiers »). À ajouter si besoin réel (soit une route admin de reset forcé, soit un flux email — aucune infra mail n'existe dans l'app pour l'instant).
- Limitations transverses à connaître :
  ...
  - Plusieurs routes existantes n'ont jamais été testées de bout en bout dans cette session : `temps.js`, `evenements.js`, `communications.js`, `documents.js`, `dashboard.js` — à auditer avant usage réel (cf. bug récurrent ci-dessous)
