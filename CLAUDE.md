...
- Modules développés (backend + écrans) : Clients & KYC (registre, fiche 360°, pièces KYC avec alertes d'expiration, originaux confiés) ; Rôle d'audience (agenda hebdomadaire, validation/diffusion, retours d'audience avec renvoi automatique au rôle suivant) ; Registre du courrier (arrivée/départ, référencement auto, déclenchement automatique d'événements/diligences/tâches selon la nature du courrier) ; Atelier d'actes (génération d'actes à partir de modèles internes avec en-tête cabinet, ou brouillon Assistant IA — enregistré dans la GED, toujours « projet à valider »).
- Modules restant à développer (backend + écrans) : Bibliothèque, Plan d'action, Dépenses & caisse, Rétrocessions, Accès & permissions, Cabinet (RH), Assistant IA (écran dédié), Portail client. Voir `APP/README.md` § 5-6.
- Note : le schéma SQL ne comporte pas de table dédiée « modèles/bibliothèque » — l'Atelier d'actes utilise des modèles codés en dur côté serveur (`backend/src/routes/actes.js`, objet `MODELES`) plutôt qu'une table paramétrable. À revoir si le module Bibliothèque introduit une vraie table de modèles réutilisables.
- ⚠️ Motif récurrent de bug rencontré : dans ce codebase (SQL brut, pas d'ORM), `COALESCE($n, 'litteral')` ou `CASE WHEN $n = 'litteral' THEN…` sur une colonne de type ENUM Postgres provoque une erreur de typage (« column is of type X but expression is of type text ») dès que le même paramètre `$n` est réutilisé dans un contexte non typé. Toujours caster explicitement (`$n::mon_enum`) ou dupliquer le paramètre plutôt que de le réutiliser tel quel. Plusieurs occurrences déjà corrigées (`dossiers.js`, `audiences.js`, `courriers.js`) — vérifier ce pattern avant d'écrire de nouvelles routes.
